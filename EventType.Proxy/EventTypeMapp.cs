...
{
    public class EventTypeMapp
    {
        public static List<EventTypeObj> MappEventType(DataTable dto)
        {
            if (dto == null)
                throw new ArgumentNullException(nameof(dto), "No fue posible obtener valores desde el DataTable.");

            var list = new List<EventTypeObj>();
            ...
            foreach (DataRow row in dto.Rows)
            {
                ...
                var item = EventTypeObj.Create(id);
                if (item != null)
                {
                    item.SetInformacion(nombre, descripcion);
