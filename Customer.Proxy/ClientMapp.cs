...
{
    public class ClientMapp
    {
        public static List<ClientObj> MappCliente(DataTable dto)
        {
            if (dto == null)
                throw new ArgumentNullException(nameof(dto), "No fue posible obtener valores desde el DataTable.");

            var list = new List<ClientObj>();
            ...
            foreach (DataRow row in dto.Rows)
            {
                ...
                var item = ClientObj.Create(id);
                if (item != null)
                {
                    item.SetInformacionBasica(nombre, telefono, correo);
